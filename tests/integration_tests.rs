...
fn test_3m_2015_10k() -> Result<()> {
    ...
    let template_path = "10k.tmpl"; // Corrected path assuming it's in the workspace root
    ...
    assert!(Path::new(&pdf_full_path).exists(), "PDF file not found at {:?}", pdf_full_path);
    ...
    assert!(Path::new(&template_full_path).exists(), "Template file not found at {:?}", template_full_path);
    let template_str = fs::read_to_string(&template_full_path)?;
    println!("Successfully read template file.");

    // Call the core processing function
    // Ignore blocks and doc for this test, only compare JSON output
    ...
    println!("Attempting to read expected output from: {:?}", expected_output_full_path);
    assert!(Path::new(&expected_output_full_path).exists(), "Expected output file not found at {:?}", expected_output_full_path);
    let expected_json_str = fs::read_to_string(&expected_output_full_path)?;
    println!("Successfully read expected output file.");

    // Deserialize actual and expected JSON
    ...
    // Assert that the JSON values are equal
    assert_eq!(actual_value, expected_value, "Processed PDF output does not match expected output.");
