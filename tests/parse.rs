...
fn test_coordinate_transformations() {
    ...
    let text_elements: Vec<TextElement> = pages_map.values()
        .flatten()
        .filter_map(|content| {
            if let PageContent::Text(text_elem) = content {
    ...
    assert!(!text_elements.is_empty(), "Should have extracted text elements");
    ...
    for element in &text_elements {
        ...
        assert!(y0 <= y1, "y0 should be less than or equal to y1 (top-left system)");
        ...
        assert!(x0 >= 0.0 && x1 <= 612.0, "x coordinates should be within page width");
        assert!(y0 >= 0.0 && y1 <= 792.0, "y coordinates should be within page height");
