use delver_pdf::docql::{process_matched_content, Element, ElementType, ProcessedOutput, Value};
use delver_pdf::matcher::{align_template_with_content, MatchedContent, TemplateContentMatch};
use delver_pdf::parse::{PageContent, TextElement};
...
fn test_real_nested_sections_parent_references() {
    ...
    for (i, output) in processed_outputs.iter().enumerate() {
        match output {
            delver_pdf::docql::ProcessedOutput::Text(chunk) => {
                println!(
                    "Output[{}]: '{}' -> parent: {:?} @ {:?}",
                    i,
            ...
            delver_pdf::docql::ProcessedOutput::Image(img) => {
                println!("Output[{}]: Image {}", i, img.id);
            }
        }
    }
    ...
    let text_outputs: Vec<_> = processed_outputs
        .iter()
        .filter_map(|output| match output {
            delver_pdf::docql::ProcessedOutput::Text(chunk) => Some(chunk),
            _ => None,
        })
        .collect();
