...
pub fn show_match_panel(viewer: &mut Viewer, ctx: &egui::Context) {
...
fn display_template_structure(viewer: &Viewer, ui: &mut egui::Ui, template_id: Uuid) {
    ...
    if let Some(structure) = viewer.debug_data.get_template_structure(template_id) {
        ...
        ui.indent("structure", |ui| {
            ...
            for (_i, item) in structure.iter().enumerate() {
...
fn navigate_to_match(viewer: &mut Viewer, content_id: Uuid) {
    ...
    for block in &viewer.blocks {
        for line in &block.lines {
            if line.id == content_id {
                ...
                viewer.selected_bbox = Some((line.bbox.0, line.bbox.1, line.bbox.2, line.bbox.3));
...
fn collect_template_matches(viewer: &Viewer) -> HashMap<Uuid, Vec<(Uuid, f32)>> {
    ...
    let templates_lock = viewer.debug_data.get_templates();
...
fn count_all_template_matches(store: &DebugDataStore) -> usize {
    store.debug_dump_all_matches().len()
}
...
fn get_content_text(viewer: &Viewer, line_id: Uuid) -> Option<String> {
...
fn count_content_items(viewer: &Viewer) -> usize {
    let mut count = 0;
    for block in &viewer.blocks {
