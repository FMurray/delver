...
fn test_detect_headings() {
    ...
    let index = delver_core::search_index::PdfIndex::new(&pages_map, &match_context);
    ...
    let expected_heading_font_name =
        delver_core::fonts::canonicalize::canonicalize_font_name(&config.font_name);
    assert_eq!(
