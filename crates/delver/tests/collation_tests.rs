...
use delver_core::matcher::{
    align_template_with_content,TemplateContentMatch,
};
...
mod collation_flow_tests {
    ...
    fn test_multi_page_chunking_preserves_page_numbers() {
        ...
        // Page 1 content
        let _page1_text1_id = doc.add_text(1, "Page one first paragraph with some content.", 12.0, 50.0, 700.0);
        let _page1_text2_id = doc.add_text(1, "Page one second paragraph continues here.", 12.0, 50.0, 680.0);
        
        // Page 2 content  
        let _page2_text1_id = doc.add_text(2, "Page two first paragraph with different content.", 12.0, 50.0, 700.0);
        let _page2_text2_id = doc.add_text(2, "Page two second paragraph also continues here.", 12.0, 50.0, 680.0);
        
        // Page 3 content
        let _page3_text1_id = doc.add_text(3, "Page three first paragraph with more content.", 12.0, 50.0, 700.0);
        let _page3_text2_id = doc.add_text(3, "Page three second paragraph concludes the document.", 12.0, 50.0, 680.0);
        ...
        for output in &processed_outputs {
            if let delver_core::docql::ProcessedOutput::Text(chunk) = output {
    ...
    fn test_non_overlapping_nested_sections() {
        ...
        let _a_p_id  = doc.add_text(1, "Paragraph A.",12.0, 50.0, 730.0);

        // Sub-sections
        let a1_h_id = doc.add_text(1, "Heading A.1", 16.0, 65.0, 710.0);
        let _a1_p_id = doc.add_text(1, "Content A.1", 12.0, 65.0, 690.0);

        let a2_h_id = doc.add_text(1, "Heading A.2", 16.0, 65.0, 670.0);
        let _a2_p_id = doc.add_text(1, "Content A.2", 12.0, 65.0, 650.0);
    ...
    fn test_overlapping_nested_sibling_boundaries() {
        ...
        let _shared_content_id = doc.add_text(1, "This content is shared between subsections", 12.0, 50.0, 780.0);
        
        // Subsection 1 starts here
        let sub1_h_id = doc.add_text(1, "Subsection One", 14.0, 60.0, 760.0);
        let _sub1_content_id = doc.add_text(1, "Content specific to subsection one", 12.0, 60.0, 740.0);
        
        // Overlapping content - both subsections could claim this
        let _overlap_content_id = doc.add_text(1, "Overlapping content area", 12.0, 50.0, 720.0);
        ...
        let _sub2_content_id = doc.add_text(1, "Content specific to subsection two", 12.0, 60.0, 680.0);
    ...
    fn test_many_overlapping_sections() {
        ...
        let _main_h_id = doc.add_text(1, "Document Start", 18.0, 50.0, 900.0);
        ...
        let _doc_end_id = doc.add_text(1, "Document End", 18.0, 50.0, 400.0);
        let index = doc.build();
...
mod recursion_detection_tests {
    ...
    fn test_non_advancing_section_boundaries() {
        ...
        let _text1_id = doc.add_text(1, "Ambiguous Section Header", 16.0, 50.0, 700.0);
        let _text2_id = doc.add_text(1, "Ambiguous Section Header", 16.0, 50.0, 700.0);
        let _content_id = doc.add_text(1, "Some content here", 12.0, 50.0, 680.0);
        let index = doc.build();
    ...
    fn test_boundary_detection_termination() {
        ...
        let _start1_id = doc.add_text(1, "Chapter Start", 16.0, 50.0, 750.0);
        let _content1_id = doc.add_text(1, "Some content", 12.0, 50.0, 730.0);
        let _start2_id = doc.add_text(1, "Chapter Start", 16.0, 50.0, 710.0);  // Same as start1
        let _content2_id = doc.add_text(1, "More content", 12.0, 50.0, 690.0);
        let _end_marker_id = doc.add_text(1, "Chapter Start", 16.0, 50.0, 670.0);  // Same pattern again
        let index = doc.build();
    ...
    fn test_get_next_match_index_always_advances() {
        ...
        let _text1_id = doc.add_text(1, "First section", 16.0, 50.0, 700.0);
        let _text2_id = doc.add_text(1, "Second section", 16.0, 50.0, 650.0);
        let _text3_id = doc.add_text(1, "Third section", 16.0, 50.0, 600.0);
        let index = doc.build();
