...
/// Render the PDF view for the async debug viewer
pub fn render_pdf_view_for_async(viewer: &mut crate::debug_viewer::async_app::AsyncDebugViewer, ui: &mut egui::Ui) {
    // Get the current texture
    if let Some(texture) = viewer.textures.get(viewer.current_page) {
        // Calculate view rectangle and transform
        let (rect, transform) = viewer.calculate_pdf_view_rect(ui, texture);
        ...
        // Draw grid if enabled
        if viewer.show_grid {
            draw_grid_for_async(viewer, &painter, rect, &transform);
        }

        // Draw blocks if enabled
        if viewer.show_blocks {
            draw_blocks_for_async(viewer, &painter, &transform);
        }

        // Draw lines if enabled
        if viewer.show_lines {
            draw_lines_for_async(viewer, &painter, &transform);
        }

        // Draw text if enabled
        if viewer.show_text {
            draw_text_for_async(viewer, &painter, &transform);
        }

        // Draw matches if enabled (with cached data)
        if viewer.show_matches {
            draw_matches_for_async(viewer, &painter, &transform);
        }

        // Handle element selection
        if response.clicked() {
            if let Some(mouse_pos) = response.hover_pos() {
                select_element_at_position_for_async(viewer, mouse_pos, &transform);
            }
        }
    } else {
        ui.centered_and_justified(|ui| {
...
// Async versions of drawing functions
fn draw_grid_for_async(
    viewer: &crate::debug_viewer::async_app::AsyncDebugViewer,
    painter: &egui::Painter,
    rect: egui::Rect,
    transform: &utils::ViewTransform,
) {
...
fn draw_matches_for_async(
    _viewer: &crate::debug_viewer::async_app::AsyncDebugViewer,
    _painter: &egui::Painter,
    _transform: &utils::ViewTransform,
) {
    // For now, skip drawing matches since we need async access to cached data
    // This could be enhanced by passing cached match data as a parameter
}

fn select_element_at_position_for_async(
    viewer: &mut crate::debug_viewer::async_app::AsyncDebugViewer,
    pos: egui::Pos2,
    transform: &utils::ViewTransform,
) {
    // Convert screen position to PDF coordinates
        ...
        if block.page_number as usize == viewer.current_page + 1 {
            for line in &block.lines {
                    ...
                    && pdf_y <= line.bbox.3
                {
                    ...
                    // Request entity events for this line
                    viewer.request_entity_events(line.id);
